...
model = torch.hub.load('ultralytics/yolov5', 'custom', path='C:/Users/aleen/PycharmProjects/fermiLabProject/best.pt')
...
try:
    while True:
        ...
        # Using trained model for per video frame
        results = model(color_image)

        # Checking if there is bolt in frame
        if results.xyxy[0].shape[0] == 0:
            cv2.imshow('YOLO', np.squeeze(results.render()))
        else:
            # calling function to find bolt center
